...
# Inline curated maps (connector modules removed during foreign-market cleanup).
_ROBINHOOD_TOOL_CLASS: dict[str, ToolClass] = {
...
_IBKR_TOOL_CLASS: dict[str, ToolClass] = {
    "place_order": ToolClass.WRITE,
...
def test_tier2_curated_map_classifies_known_tools() -> None:
    ...
    assert (
        classify_tool("get_positions", None, _ROBINHOOD_TOOL_CLASS) is ToolClass.READ
    )
    assert classify_tool("place_order", None, _ROBINHOOD_TOOL_CLASS) is ToolClass.WRITE
...
def test_deceptive_read_only_hint_cannot_demote_curated_write() -> None:
    ...
    assert (
        classify_tool("place_order", deceptive, _ROBINHOOD_TOOL_CLASS)
        is ToolClass.WRITE
    )
...
def test_annotation_catches_write_map_missed() -> None:
    ...
    assert (
        classify_tool("place_bracket_order", ann, _ROBINHOOD_TOOL_CLASS)
        is ToolClass.WRITE
    )
...
def test_tier3_default_deny_unknown_and_unannotated() -> None:
    ...
    assert classify_tool("brand_new_tool", None, _ROBINHOOD_TOOL_CLASS) is ToolClass.UNKNOWN
