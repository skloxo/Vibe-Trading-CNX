...
- **2026-06-13** 🌏 **前端 i18n — 中文（zh-CN）本地化**：Web UI 现已支持中文（zh-CN），基于 i18next 的完整国际化层。所有 20+ 组件和页面使用 `useTranslation()` hook 实现响应式语言切换，449 个翻译键覆盖导航、聊天、设置、图表和错误消息。侧边栏单个切换按钮可即时切换中英文。翻译文件：`src/i18n/locales/zh-CN.json` 和 `en.json`（[#207](https://github.com/HKUDS/Vibe-Trading/pull/207)，感谢 @skloxo）。
